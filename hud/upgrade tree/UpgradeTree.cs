...
public partial class UpgradeTree : Node
{
    ...
    Dictionary<Improvement, List<Improvement>> childrenLookup; // The value is a list of everything that is (partially or fully) unlocked by the key
    ...
    public override void _Ready()
    {
        ...
        List<Improvement> allStatsAndUnlocks = GetDefaultStats().ToList<Improvement>(); // Just grabs the default stats for now
        foreach (Unlockable u in Unlocks.allUnlockables)
        {
            allStatsAndUnlocks.Add(u);
            foreach (Improvement stat in u.associatedStats.upgradeableStats)
            {
        ...
        List<Improvement> baseUpgrades = new List<Improvement>();

        childrenLookup = new Dictionary<Improvement, List<Improvement>>();

        foreach (Improvement p in allStatsAndUnlocks)
        {
            UpgradeTreeNode n = nameLookup[p.GetName()];
            List<Improvement> prereqs = p.GetPrerequisites();
            if (prereqs is null)
            {
            ...
            foreach (Improvement unlocker in prereqs)
            {
    ...
    void PositionNodeAndChildren(Improvement root, Vector2 direction)
    {
        ...
        foreach (Improvement p in childrenLookup[root])
        {
            PositionNodeAndChildren(p, direction + direction.Normalized() * linkRestDistance);
        }
    }

    void AddRequirement(Improvement unlocker, Improvement unlockee)
    {
        if (!childrenLookup.Keys.Contains(unlocker))
        {
            childrenLookup[unlocker] = new List<Improvement> { unlockee };
        }
        else
        {
    ...
    void PrintPrereqs(Improvement prereq)
    {
        List<Improvement> prereqs = prereq.GetPrerequisites();
        List<string> s;
    ...
    UpgradeTreeNode SpawnNode(Improvement p)
    {
    ...
    List<UpgradeTreeNode> SpawnPrereqs(Improvement prereq)
    {
        List<Improvement> prereqs = prereq.GetPrerequisites();
